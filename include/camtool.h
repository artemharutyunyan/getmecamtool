...
typedef struct webui_file_header_t {
	...
	int32_t         checksum;	/* !< Checksum (sum of all bytes starting
					 * from offset 12) */
...
=======
//!Header for files
that are packed into Web UI data blob typedef struct webui_fentry_t {
	int32_t         name_size;	/* !< Size of file name string */
	char           *name;	/* !< Name of the file */
	char            type;	/* !< Type of the entry (set 01 in case of files) */
	int32_t         size;	/* !< Size of the file */
...
that are packed into Web UI data blob typedef struct webui_dentry_t {
	int32_t         name_size;	/* !< Size of file name string */
	char           *name;	/* !< Name of the file */
	char            type;	/* !< Type of the entry (set 00 in case of
				 * directories) */
} webui_dentry;
//!Header for files
and directories hat are packed into Web UI data blob typedef struct webui_entry_t {
	int32_t         name_size;	/* !< Size of file name string */
	char            name[MAX_FILE_NAME_LEN];	/* !< Name of the
							 * file/directory */
	char            type;	/* !< Type of the entry (set 01 in case of files) */
	int32_t         size;	/* !< Size of the file */
	char            data[MAX_FILE_SIZE];	/* !< File data */
